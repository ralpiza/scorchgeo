...
class Map(ipyleaflet.Map):
    ...
    def add_shp(self, data, layer_name=None, hover_style=None, **kwargs):
        ...
        if isinstance(data, str):
            ...
            geojson = data
        layer = ipyleaflet.GeoJSON(data=geojson, name=layer_name, hover_style=hover_style, **kwargs)
        self.add_layer(layer)
    ...
    def add_vector(self, data, hover_style=None, zoom_to_layer=True, **kwargs):
        ...
        if isinstance(data, str):   
            gdf = gpd.read_file(data)
            self.add_gdf(gdf, hover_style=hover_style, zoom_to_layer=zoom_to_layer, **kwargs)
        elif isinstance(data, gpd.GeoDataFrame):
            self.add_gdf(data, hover_style=hover_style, zoom_to_layer=zoom_to_layer, **kwargs)
        elif isinstance(data, dict):
            self.add_geojson(data, hover_style=hover_style, zoom_to_layer=zoom_to_layer, **kwargs)
        else:
            raise ValueError("Unsupported data type. Please provide a file path, GeoDataFrame, or GeoJSON dictionary.")